...
Note:
When registering a Member, the system automatically creates or updates a corresponding Customer record using the provided name and phone number (if supplied).
...
Recommended approach is to deactivate the user/customer instead of deleting.
...
Read operations are public only for endpoints explicitly marked with [AllowAnonymous].

Method	Endpoint	Description	Access
GET	/	Get all tools (with filters)	Public
GET	/{id}	Get tool by ID	Public
GET	/{id}/available	Check availability	Public
GET	/status/{status}	Get tools by status	Public
...
Seeded customer/member accounts:
...
Admin-only endpoints are explicitly restricted using role-based authorization

Ownership checks (e.g. accessing or modifying rentals) are enforced in the service layer

Summary

TooliRentB demonstrates:
